...
while True:
    for url in URLS:
        ...
        availability_class_list = ("availabilityText ZZaj", "availabilityText ZZek")

        for class_id in availability_class_list:

            # Getting item name & availability status
            span_title = soup.find_all(
                "span", {"class": "productHeaderTitle__Name-gtvrqo-1 iJLJgg"}
            )
            span_data = soup.find_all("span", {"class": class_id})
            ...
            for span_tag in span_data:
                ...
                if (
                    ...
                    print(f'(Value found: "{span_tag.text}")')
                else:
    ...
    print(
        f"\n[{DT_OBJECT}]: No available items found - Pausing for 30 seconds before next iteration...\n\n"
    )
    time.sleep(30)
